...
| **`isExactNotification`** | <code>boolean</code>                                            | Whether this notification should be scheduled with an exact alarm. Only available for Android. Defaults to `true`: the plugin always attempts an exact alarm and falls back to an inexact one if the exact-alarm permission isn't granted (unless `isExactMandatory` is also set). On `schedule()`, a fallback like this sets <a href="#scheduleresult">`ScheduleResult.warning`</a>; on `update()` it happens silently. Set to `false` to schedule this notification as inexact outright, regardless of permission state.                                                                                                 | <code>true</code>  | 8.3.0 |
