...
publish_plugin_ios () {
    ...
    # PascalCase the name (podspec filenames are PascalCase, e.g. "local-notifications" -> "LocalNotifications")
    PLUGIN_NAME=$(echo "$PLUGIN_NAME" | awk -F- '{for(i=1;i<=NF;i++){$i=toupper(substr($i,1,1)) substr($i,2)}}1' OFS='')
